        ...
        <div class="rounded-xl border border-neutral-200 bg-white p-8 shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
            <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        @php($selectedCourseId = old('course_id', $testimonial->course_id))
                        ...
                        <select name="course_id" class="mt-1 block w-full rounded-md border-neutral-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white h-10 px-3">
                            <option value="">No verified course link{{ filled($testimonial->course_name) ? ' — preserve legacy label: '.$testimonial->course_name : ' (general academy testimonial)' }}</option>
